                ...
                <div>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li>{!! HTML::link('lang/en', trans('menus.language-picker.lanrgs.en')) !!}</li>
                            <li>{!! HTML::link('lang/am', trans('menus.language-picker.langs.am')) !!}</li>
                        </ul>
                    </li>
                </div>
            </div>
...
<header id="page-header-bottom" class="page-header-bottom">
    <div class="grid-row">
        ...
        <nav class="main-nav">
            <ul>
                <li>
                    {!! HTML::link('/', trans('navs.home')) !!}

                </li>
                <li>
                    {!! HTML::link('/category', trans('innovate.menus.category')) !!}

                </li>
                <li>
                    {!! HTML::link('/product', trans('innovate.menus.product')) !!}

                </li>
                <li>
                    {!! HTML::link('/product/trend', trans('innovate.menus.trend_products')) !!}
                </li>

                    @if (Auth::guest())
                <li>{!! HTML::link('auth/login', trans('navs.login')) !!}</li>
                <li>{!! HTML::link('auth/register', trans('navs.register')) !!}</li>
                @else
                    <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">{{ Auth::user()->name }} <span class="caret"></span></a>
                        <ul class="dropdown-menu" role="menu">
                            <li>{!! HTML::link('activity_logs', trans('activity.navs.activity_log')) !!}</li>

                            @if (access()->user()->canChangePassword())
                                <li>{!! HTML::link('auth/password/change', trans('navs.change_password')) !!}</li>
                            @endif

                            @permission('view-backend')
                            <li>{!! url_route('admin.dashboard', trans('navs.administration')) !!}</li>
                            @endauth

                            <li>{!! HTML::link('auth/logout', trans('navs.logout')) !!}</li>
                        </ul>
                    </li>
                    @endif
